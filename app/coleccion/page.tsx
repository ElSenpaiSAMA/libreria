...
import { Library, SlidersHorizontal, Search as SearchIcon } from 'lucide-react';
import { fetchBooks, searchBooks, fetchBooksByCategory } from '@/lib/open-library-api';
import type { Book } from '@/types';

function removeDuplicateBooks(books: Book[]): Book[] {
      ...
      try {
        ...
        if (searchQuery) {
          ...
          const results = await Promise.all(
            queries.map(query => fetchBooks(query, 10))
          );
          booksData = removeDuplicateBooks(results.flat());
        }
          ...
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              ...
              <p className="text-xl md:text-2xl text-amber-100 mt-3 drop-shadow-md">
                Explora nuestra biblioteca de {books.length} libros
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-6">
          <h3 className="text-2xl font-bold text-gray-900 mb-2">
            {selectedCategory === 'Todos' ? 'Todos los Libros' : selectedCategory}
          </h3>
